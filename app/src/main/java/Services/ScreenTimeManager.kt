...
class ScreenTimeManager(
            ...
            .map {
                ...
                if (appLabel == it.key || (appLabel.contains(".") && appLabel.lowercase() == appLabel)) {
                    ...
                    appLabel = if (parts.size >= 2) {
                        ...
                        // Handle com.instagram.android -> Instagram
                        if (name.equals("android", ignoreCase = true) && parts.size >= 3) {
                            name = parts[parts.size - 2]
                        } else if (name.length <= 3 && parts.size >= 3) {
                            name = parts[parts.size - 2]
                        }
     ...
     * To match Digital Wellbeing exactly, we rely primarily on 
     * Android's aggregated UsageStats.
     */
    private fun applyHybridCorrection(
        eventValue: Long,
        statsValue: Long
    ): Long {
        // Digital Wellbeing tallies based on totalTimeInForeground
        // Returning statsValue directly ensures it matches the system reports.
        return if (statsValue > 0) statsValue else eventValue
    }
    ...
    fun getUsageForWindow(startTime: Instant, endTime: Instant): Long {
        if (!hasPermission()) return 0L

        val usage = collectUsageEvents(
